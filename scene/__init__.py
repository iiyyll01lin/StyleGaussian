...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None,  
        ...
        if load_path:
            print("Loading gaussians from {}".format(load_path))
        elif load_iteration:
            if load_iteration == -1:
        ...
        if load_path:
            ...
            if extension == ".ply":
                ...
                if style_model:
                    self.gaussians.restore(torch.load(load_path), from_style_model=True)
                else:
                    self.gaussians.restore(torch.load(load_path), from_feature_model=True)
